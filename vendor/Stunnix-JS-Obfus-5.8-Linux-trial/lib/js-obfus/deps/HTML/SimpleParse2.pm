...
#    } elsif ($$text =~ /\G<(([a-zA-Z][a-zA-Z0-9:\.\-]*)([^<]*?))((\/?>)|(?=<))/gcs) { #orig vers
     } elsif ($$text =~ /\G<((
		([a-zA-Z][a-zA-Z0-9:\.\-]*)
     ( \s+ #all args here
       (?: 
        (?:
	(?:[\w:.\0-]+)\s*=\s*                         # the key with assignment
	 (?:
	...
	| (?:[\w.\0-]+)\s*  #bare word
      ...
      $content = $2; $type = 'starttag';

      $elt = $3; 
      my $attrs = $4;
      my $sfx = $5; 
#print STDERR "got elt |$elt| sfx |$sfx| atrs |$attrs| cont |$content|\n";
      push @otherargs, 'attributes', $attrs, 'suffix', $sfx;
    } elsif ($$text =~ /\G<%(.*?)%>/gcs) {
      $content = $1;
      my $t = substr($content,0,1);
      my $pfx = '';
      if ($t eq '#' or $t eq '=') {
	$content = substr($content,1);
	$pfx = $t;
      }
      push @otherargs, ('prefix','%',  'suffix','%',  'subtype',$pfx);
      $type = 'aspblock';
    } elsif ($$text =~ /\G([<>])/gcs) {
      #we have broken html. Handle it as text
      $content = $1; $type = 'text';
    } else {
    ...
    if (length($elt))
    {
	$elt = ($fix_case==1 ? uc($elt) : $fix_case==-1 ? lc($elt) : $elt);
	push @otherargs, 'element', $elt;
    };    
    push @$tree, {
		  'content' => $content,
		  'type'    => $type,
		  'offset'  => ($type eq 'text' ? 
		   ...
		   @otherargs #'attributes' and 'element'
		 };
  }
  ...
  while (1) {
    ...
    if ( $str =~ m/\G
	 ...
	 (?:
      ...
      my ($key, $val) = ($1, $+);
#      $val =~ s/\\(.)/$1/gs; #commented out. What was it for?
      push @returns, ($fix_case==1 ? uc($key) : $fix_case==-1 ? lc($key) : $key), $val;
      
    } elsif ( $str =~ m,\G/?([\w.\0-]+)\s*,gc ) {
