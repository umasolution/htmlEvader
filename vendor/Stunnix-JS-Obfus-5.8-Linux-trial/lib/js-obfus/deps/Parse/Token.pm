...
$TEMPLATE{'LEXEVENT_HEADER_STREAM_PART'} = q@
	...
	} until ($tmp =~ /^(?:%%"$REGEXP_MIDDLE$REGEXP_END"%%)/sg); # don't forget /g
      }
      $LEX_POS = $start_pos + pos($tmp);
      $LEX_OFFSET += $LEX_POS;
      if ($line_read) {
...
####the line above cause it not to work with Perl 5.10.x, writing it the way below solves
####the problem and doesn't cause problems with older versions of perl
